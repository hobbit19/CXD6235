...
def get_branch(response):
	...
	m = re.match(p, response)
	if m:
		url = "https://172.24.147.72:7003/webservices/10/2/Integrity/?wsdl"
		username = "hz.int"
		password = "ptc"
...
def check_state_in_response(response):
	return (">Delivered<" in response or ">Verified_SW<" in response or ">Verified<" in response or ">Closed<" in response or ">Postponed<" in response or ">Deleted<" in response)
