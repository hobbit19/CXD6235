...
public class JRDRapi {
	...
	private File prog = new File("/system/bin/jrd_rapiproxy");

	private static JrdProxyServerThread mProxyServerInstance;

	public JRDRapi(Context context) throws FileNotFoundException {

		try {
			mPipeName = context.getFileStreamPath("fifo").getAbsolutePath();
		} catch (SecurityException e) {
			Log.d(TAG, e.toString());
		}

		if (!prog.exists()) {
			prog = context.getFileStreamPath("rapi");// default prog is not
														// here, look
			// in local dir
			...
			if (!prog.exists()) {
				...
				try {
					InputStream fd = context.getResources().getAssets()
							.open("jrd_rapiproxy");
					...
					Log.d(TAG, "read " + len + " bytes of jrd_rapiproxy into "
							+ prog.getName());
					...
					if (e.getClass() == IOException.class)
						Log.e(TAG, "can't open jrd_rapiproxy from assets");
					...
					throw new FileNotFoundException(
							"/system/bin/jrd_rapiproxy nor " + prog.getName()
									+ " exist");
				}
			}
		} else {
		...
		if (mProxyServerInstance == null)
			try {
				...
				Log.e(TAG, "JrdProxyServerThread init error : " + e);
				return;
			}
	...
	class CallbackThread extends Thread {
		...
		public void run() {
			switch (structFlag) {
			case 0:
				mResult = sendcmd(mPipeName, para[0], para[1], para[2], para[3]);
				Log.d(TAG, "sendcmd returned " + mResult);
				break;
			case 1:
				this.sb.append(sendcmdForStrResult(mPipeName, para[0], para[1],
						para[2], para[3]));
				Log.d(TAG, "sendcmdForStrResult returned " + this.sb);
				break;
			default:
				break;
			}
	...
	/*                                               */
	/* JrdProxyServerThread executes jrd_rapiproxy */
	/* and polls its output for result */

	private class JrdProxyServerThread extends Thread {

		private boolean poll;

		private Process proc;

		private JrdProxyServerThread() throws SecurityException, IOException,
				FileNotFoundException {

			if (!prog.exists()) {
				throw new FileNotFoundException("JrdProxyServer executable : "
			...
			// jrd_rapiproxy creates two pipes _read and _write
