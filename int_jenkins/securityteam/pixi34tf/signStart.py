...
class sign_apk():
	...
	def replace(self):
		docmd('cp /local/int_tools/securityteam/pixi34tf/sign_target_files_apks.py .')
		docmd('cp /local/int_tools/securityteam/pixi34tf/jrd_magic.pl mediatek/build/tools/')
                os.chdir('build/target/product')
                docmd('zip -r security_bc.zip security')
                os.chdir(self.tmpdir)
		docmd('rm -rf build/target/product/security/*')
		docmd('cp /local/int_tools/securityteam/pixi34tf/TCL_ReleaseKeys.zip build/target/product/security/')
