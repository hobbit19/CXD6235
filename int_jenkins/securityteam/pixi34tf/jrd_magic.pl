...
sub add_magic {
    ...
    ## PR 928299 add recovery version for img add by jianbo.deng
    if (-e "$outdir/recovery.img") {
        open(MY_FILE, "+<$outdir/recovery/root/recovery.ver");
        $recovery_pattern = <MY_FILE>;
        close(MY_FILE);
        open FP, "+<$outdir/recovery.img" or die "can't open recovery image!\n";
        binmode FP;
        seek FP,0,2 or die $!;
        print FP $recovery_pattern;
        print "recovery.img signed.\n";
        close FP;
        }

}
#FR443123 added this file by yongming.li for root check
sub usage {
