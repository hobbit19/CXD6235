...
class sign_apk():
	...
	def replace(self):
		docmd('cp /local/int_tools/securityteam/pixi34-telus/sign_target_files_apks.py .')
		docmd('cp /local/int_tools/securityteam/pixi34-telus/jrd_magic.pl mediatek/build/tools/')
                os.chdir('build/target/product')
                docmd('zip -r security_bc.zip security')
                os.chdir(self.tmpdir)
		docmd('rm -rf build/target/product/security/*')
		docmd('cp /local/int_tools/securityteam/pixi34-telus/TCL_ReleaseKeys.zip build/target/product/security/')
...
if __name__ == "__main__":
	...
	#	sys.exit(1)
	dist = sign_apk(sys.argv[1], 'pixi34_tf')
	#dist = sign_apk('')
        dist.build()
	dist.replace()
	dist.signapk()
