...
class sign_apk():
	...
	def replace(self):
		docmd('cp /local/int_tools/securityteam/yaris35/misc_sign_yariss_custpack.sh .')
                os.chdir('build/target/product')
                docmd('zip -r security_bc.zip security')
                os.chdir(self.tmpdir)
		docmd('rm -rf build/target/product/security/*')
		docmd('cp /local/int_tools/securityteam/yaris35/TCL_ReleaseKeys.zip build/target/product/security/')
		os.chdir('build/target/product/security')
		docmd('unzip TCL_ReleaseKeys.zip')
	
	def build(self):

		docmd('./makeMtk -t -o=TARGET_BUILD_VARIANT=user yariss dist')
	...
	def signapk(self):
		...
		child = pexpect.spawn('bash ./misc_sign_yariss_custpack.sh out/dist/yariss-target_*.zip out/dist/sign_target_apks.zip')
		...
		os.chdir(self.tmpdir)
		docmd('mv out/target/product/yariss/custpack.img out/target/product/yariss/custpack.img_unsign')
		docmd('mv out/target/product/yariss/system.img out/target/product/yariss/system.img_unsign')
		docmd('mv out/target/product/yariss/recovery.img out/target/product/yariss/recovery.img_unsign')
		docmd('mv out/target/product/yariss/boot.img out/target/product/yariss/boot.img_unsign')
		docmd('cp out/dist/sign_img/custpack.img out/target/product/yariss/')
		docmd('cp out/dist/sign_img/system.img out/target/product/yariss/')
		docmd('cp out/dist/sign_img/recovery.img out/target/product/yariss/')
		docmd('cp out/dist/sign_img/boot.img out/target/product/yariss/')
		docmd('perl mediatek/build/tools/jrd_magic.pl yariss')
