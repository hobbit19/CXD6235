...
class sign_apk():
	...
	def replace(self):
		docmd('cp /local/int_tools/securityteam/pixi3-45-4G/sign_target_files_apks.py .')
...
#googlesecurity
		...
		docmd('cp /local/int_tools/securityteam/pixi3-45-4G/TCL_ReleaseKeys.zip build/target/product/security/')
	...
	def build(self):
		...
		docmd('cp /local/int_tools/securityteam/pixi3-45-4G/mplugin_guard.xml out/target/product/%s/system/plugin/Signatures/'%self.compilePrj)
		#docmd('make dist')
		...
		if self.version != None:
			...
			if self.version[2:3] == 'G':
				docmd('./tclMake -o=TARGET_BUILD_VARIANT=user,EMMC_WRITE_PROTECT=true %s dist' %self.compilePrj)
                        if self.version[2:3] == 'H':
				docmd('./tclMake -o=TARGET_BUILD_VARIANT=user,EMMC_WRITE_PROTECT=true %s dist' %self.compilePrj)
		else:
			docmd('./tclMake -o=TARGET_BUILD_VARIANT=user,EMMC_WRITE_PROTECT=true %s dist' %self.compilePrj)
		#pass
...
if __name__ == "__main__":
	...
	if len(sys.argv)==4:
		if sys.argv[3][2:3] == 'D':
			dist = sign_apk(sys.argv[1], 'jhz6735m_35u_l',sys.argv[3])
		if sys.argv[3][2:3] == '9':
			dist = sign_apk(sys.argv[1], 'jhz6735m_35u_l',sys.argv[3])
		if sys.argv[3][2:3] == 'G':
			dist = sign_apk(sys.argv[1], 'jhz6735m_35u_tm_l',sys.argv[3])
		if sys.argv[3][2:3] == 'H':
			dist = sign_apk(sys.argv[1], 'jhz6735m_35u_vl_l',sys.argv[3])
	else:
		dist = sign_apk(sys.argv[1], 'jhz6735m_35u_l',None)
	#dist = sign_apk('')
	#dist = sign_apk('')
	dist.build()
	dist.replace()
