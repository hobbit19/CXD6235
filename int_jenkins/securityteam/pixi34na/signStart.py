...
class sign_apk():
	...
	def replace(self):
		docmd('cp /local/int_tools/securityteam/pixi34na/sign_target_files_apks.py .')
		...
		docmd('cp /local/int_tools/securityteam/pixi34na/TCL_ReleaseKeys.zip build/target/product/security/')
...
if __name__ == "__main__":
	...
	#	sys.exit(1)
	dist = sign_apk(sys.argv[1], 'pixi34_tmous')
	#dist = sign_apk('')
	dist.build()
	dist.replace()
	dist.signapk()
