...
class sign_apk():
	...
	def replace(self):
		docmd('cp /local/int_tools/securityteam/pixi335tf/sign_target_files_apks.py .')
		docmd('cp /local/int_tools/securityteam/pixi335tf/jrd_magic.pl mediatek/build/tools/')
                os.chdir('build/target/product')
                docmd('zip -r security_bc.zip security')
                os.chdir(self.tmpdir)
		docmd('rm -rf build/target/product/security/*')
		if self.version[1:2] == 'D':
			docmd('cp /local/int_tools/securityteam/pixi335tf/TCL_ReleaseKeys_TF.zip build/target/product/security/')
                	os.chdir('build/target/product/security')
                	docmd('unzip TCL_ReleaseKeys_TF.zip')
		else:
			docmd('cp /local/int_tools/securityteam/pixi335tf/TCL_ReleaseKeys.zip build/target/product/security/')
			os.chdir('build/target/product/security')
...
if __name__ == "__main__":
	...
	#	sys.exit(1)
	dist = sign_apk(sys.argv[1], 'pixi335_tf', sys.argv[3])
	#dist = sign_apk('')
	dist.build()
	dist.replace()
	dist.signapk()
