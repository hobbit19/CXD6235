...
platformKeyApkInCustpack = ['framework-res.apk','mediatek-res.apk','Jrdshared.apk']
#platformKeyApkInCustpack = []

#Apk in system need to sign other keys which not match that in META/certapks.txt 
indicateKey= ''

content = getoutput('unzip -lv %s | grep CUSTPACK\/ | grep apk | grep Defl:N' %input_package)
for line in content.split('\n'):
	if line.strip() == '':
		continue
	apkDict[line.split('/')[-1]] = ''

for apk in platformKeyApkInCustpack:
