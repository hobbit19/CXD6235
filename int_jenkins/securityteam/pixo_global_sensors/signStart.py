...
class sign_apk():
	...
	def replace(self):
		docmd('cp /local/int_tools/securityteam/pixo_global_sensors/misc_sign_pixo_custpack.sh .')
                os.chdir('build/target/product')
                docmd('zip -r security_bc.zip security')
                os.chdir(self.tmpdir)
		docmd('rm -rf build/target/product/security/*')
		docmd('cp /local/int_tools/securityteam/pixo_global_sensors/TCT_ReleaseKeys.zip build/target/product/security/')
		os.chdir('build/target/product/security')
		docmd('unzip TCT_ReleaseKeys.zip')
	
	def build(self):

		docmd('./makeMtk -t -o=TARGET_BUILD_VARIANT=user jrdhz75_gb2_sensors dist')
	...
	def signapk(self):
		...
		child = pexpect.spawn('bash ./misc_sign_pixo_custpack.sh out/dist/jrdhz75_gb2_sensors-target_*.zip out/dist/sign_target_apks.zip')
		...
		os.chdir(self.tmpdir)
		docmd('mv out/target/product/jrdhz75_gb2_sensors/custpack.img out/target/product/jrdhz75_gb2_sensors/custpack.img_unsign')
		docmd('mv out/target/product/jrdhz75_gb2_sensors/system.img out/target/product/jrdhz75_gb2_sensors/system.img_unsign')
		docmd('mv out/target/product/jrdhz75_gb2_sensors/recovery.img out/target/product/jrdhz75_gb2_sensors/recovery.img_unsign')
		docmd('mv out/target/product/jrdhz75_gb2_sensors/boot.img out/target/product/jrdhz75_gb2_sensors/boot.img_unsign')
		docmd('cp out/dist/sign_img/custpack.img out/target/product/jrdhz75_gb2_sensors/')
		docmd('cp out/dist/sign_img/system.img out/target/product/jrdhz75_gb2_sensors/')
		docmd('cp out/dist/sign_img/recovery.img out/target/product/jrdhz75_gb2_sensors/')
		docmd('cp out/dist/sign_img/boot.img out/target/product/jrdhz75_gb2_sensors/')
