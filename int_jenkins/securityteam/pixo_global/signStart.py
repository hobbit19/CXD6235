...
class sign_apk():
	...
	def replace(self):
		docmd('cp /local/int_tools/securityteam/pixo_global/misc_sign_pixo_custpack.sh .')
                os.chdir('build/target/product')
                docmd('zip -r security_bc.zip security')
                os.chdir(self.tmpdir)
		docmd('rm -rf build/target/product/security/*')
		docmd('cp /local/int_tools/securityteam/pixo_global/TCT_ReleaseKeys.zip build/target/product/security/')
	...
	def signapk(self):
		...
		child = pexpect.spawn('bash ./misc_sign_pixo_custpack.sh out/dist/jrdhz75_gb2-target_*.zip out/dist/sign_target_apks.zip')
