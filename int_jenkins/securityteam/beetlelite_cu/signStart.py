...
class sign_apk():
	...
	def replace(self):
		docmd('cp /local/int_tools/securityteam/beetlelite_cu/misc_sign_beetlelite_custpack.sh .')
                os.chdir('build/target/product')
                docmd('zip -r security_bc.zip security')
                os.chdir(self.tmpdir)
		docmd('rm -rf build/target/product/security/*')
		docmd('cp /local/int_tools/securityteam/beetlelite_cu/TCT_ReleaseKeys.zip build/target/product/security/')
		os.chdir('build/target/product/security')
		docmd('unzip TCT_ReleaseKeys.zip')
	
	def build(self):

		docmd('./makeMtk -t jrdhz75_cu_gb2 dist')
	...
	def signapk(self):
		...
		child = pexpect.spawn('bash ./misc_sign_beetlelite_custpack.sh out/dist/jrdhz75_cu_gb2-target_*.zip out/dist/sign_target_apks.zip')
		...
		os.chdir(self.tmpdir)
		#docmd('mv out/target/product/jrdhz75_cu_gb2/custpack.img out/target/product/jrdhz75_cu_gb2/custpack.img_unsign')
		docmd('mv out/target/product/jrdhz75_cu_gb2/system.img out/target/product/jrdhz75_cu_gb2/system.img_unsign')
		docmd('mv out/target/product/jrdhz75_cu_gb2/recovery.img out/target/product/jrdhz75_cu_gb2/recovery.img_unsign')
		docmd('mv out/target/product/jrdhz75_cu_gb2/boot.img out/target/product/jrdhz75_cu_gb2/boot.img_unsign')
		#docmd('cp out/dist/sign_img/custpack.img out/target/product/jrdhz75_cu_gb2/')
		docmd('cp out/dist/sign_img/system.img out/target/product/jrdhz75_cu_gb2/')
		docmd('cp out/dist/sign_img/recovery.img out/target/product/jrdhz75_cu_gb2/')
		docmd('cp out/dist/sign_img/boot.img out/target/product/jrdhz75_cu_gb2/')
