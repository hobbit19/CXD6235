...
class ModemPatchMerge(CheckFile,dotProjectDb,checkPatchInfor):
    def __init__(self,basedir='/local'):
        ...
        wb = xlrd.open_workbook('%s/mtk6580-l-rpmb-jb3-mp-import.xls' % getoutput("dirname %s" % (sys.argv[0])))
    ...
    def __takePatch(self,codedir,patchNo=''):
        ...
        os.system('git rm %s/mtk_rel/JHZ6580_WE_TEE_L_GPRS/DEFAULT/tst/database/BPLGUInfoCustomApp*' %codedir)
        os.system('git rm %s/mtk_rel/JHZ6580_WE_TEE_L_GPRS/DEFAULT/tst/database/_BPLGUInfoCustomApp*' %codedir)
        os.system('git rm %s/mtk_rel/JHZ6580_WE_TEE_L_HSPA/DEFAULT/tst/database/BPLGUInfoCustomApp*' %codedir)
        os.system('git rm %s/mtk_rel/JHZ6580_WE_TEE_L_HSPA/DEFAULT/tst/database/_BPLGUInfoCustomApp*' %codedir)
    ...
    def __compile(self, CodeDir):		
        ...
        print "building......"
        if False == self.__debug:
            buildStatus = getstatusoutput("./make.sh \"JHZ6580_WE_TEE_L_HSPA.mak\" new")
	...
	dearName = sys.argv[4].split('@')[0]
		...
		if True == self.getFileLibNvram(self.__ModemCode,projectname):
            ...
            a = u"\\\\10.92.32.12\\RDhzKM\\4-SWD1\\B-技术文档\\A-MTK源代码\\6580_WE_RPMB_L\\patch"
            mailBody.append('<p align=\'Left\'><b>You can also find this patch (P%s) in:<font color="#FF0000">%s</font></b></p>' % (patchNo, a))

            mailBody.append('<p align=\'Left\'><b>After you complete to merge the MTK6580 Pixi3-5 RPMB patch P%s,</b></p>' % patchNo)
