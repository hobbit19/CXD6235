...
class PatchMerge(CheckFile,dotProjectDb,checkPatchInfor):
	...
	self.repopath = '/local/tools/repo/repo'
        ...
        self.specialDir = ['device', 'vendor', 'kernel-3.10']
	...
	print 'patchFilename',patchFilename
        ...
        if bugInfo['patchID'] == 'JRD':
            ...
            mailBody.append('<p align=\'Left\'>MTK6580L Platform Pixi3-5 RPMB defect %s(<a href="%s">%s </a>)</p>' %(bugInfo['bugID'], bugInfo['bugLink'], bugInfo['bugLink']))                               
        if bugInfo['patchID'] == 'ALPS':
            mailBody.append('<p align=\'Left\'><b>Dear %s,</b></p>' % dearName)
            mailBody.append('<p align=\'Left\'>MTK6580L Platform Pixi3-5 RPMB Patch %s<br/></p>' %bugInfo['bugID'])
	...
	for item in range(len(patchList)):
		...
		gitName = 'kernel'
            gitLink = 'http://10.92.32.10/sdd1/gitweb-mtk6580/?p=%s.git;a=commit;h=%s' % (gitName, itemInfo[1])
	...
	mailBody.append('<p align=\'Left\'><b>After you complete to merge the MTK6580 Pixi4 patch P%s,</b></p>' % patchNo)
    ...
    def __compile(self, CodeDir, branchname):		
        ...
        print "building......"
        if False == self.__debug:
            status = os.system('source build/envsetup.sh && lunch full_jhz6580_we_tee_l-eng && make -j24 2>&1 |tee android.log')
            print "status is %s" % status            
            if os.path.exists(r"/local/mtk_patch_import/%s/out/target/product/jhz6580_we_tee_l" % branchname):
                os.chdir(r'/local/mtk_patch_import/%s/out/target/product/jhz6580_we_tee_l'  % branchname)
