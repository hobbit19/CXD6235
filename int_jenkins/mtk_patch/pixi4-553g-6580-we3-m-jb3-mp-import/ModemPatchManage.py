...
class ModemPatchMerge(CheckFile,dotProjectDb,checkPatchInfor):
    def __init__(self,basedir='/local'):
        ...
        wb = xlrd.open_workbook('%s/jb3-mp-import.xls' % getoutput("dirname %s" % (sys.argv[0])))
    ...
    def repo_code(self, CodeDir, ModemDir, CodeBranchName):
        ...
        if os.path.exists(CodeDir + "/modem" + "/.git") == False:
            os.system('git clone git@10.92.32.10:sdd3/mtk6580/modem.git modem')
            os.chdir(ModemDir)
    ...
    def __takePatch(self,codedir,patchNo=''):
        ...
        os.system('git rm %s/mtk_rel/JHZ6580_WE_3_M_GPRS/DEFAULT/tst/database/BPLGUInfoCustomApp*' %codedir)
        os.system('git rm %s/mtk_rel/JHZ6580_WE_3_M_GPRS/DEFAULT/tst/database/_BPLGUInfoCustomApp*' %codedir)
        os.system('git rm %s/mtk_rel/JHZ6580_WE_3_M_HSPA/DEFAULT/tst/database/BPLGUInfoCustomApp*' %codedir)
        os.system('git rm %s/mtk_rel/JHZ6580_WE_3_M_HSPA/DEFAULT/tst/database/_BPLGUInfoCustomApp*' %codedir)
	...
	dearName = sys.argv[4].split('@')[0]
        ...
        for item in range(len(patchList)):
            itemInfo = patchList[item].split('^^^^^^')    			
            gitNamefilder = 'modem'  		
            gitLink = 'http://10.92.32.10/sdd3/gitweb-mtk6580/?p=modem.git;a=commit;h=%s' % itemInfo[1]
		...
		if True == self.getFileLibNvram(self.__ModemCode,projectname):
            ...
            a = u"\\\\10.92.32.12\\RDhzKM\\SWD-Share\\INT\\MTKPatch\\pixi4-55-3g"
            mailBody.append('<p align=\'Left\'><b>You can also find this patch (P%s) in:<font color="#FF0000">%s</font></b></p>' % (patchNo, a))

            mailBody.append('<p align=\'Left\'><b>After you complete to merge the MTK6580 pixi4-553G patch P%s,</b></p>' % patchNo)
