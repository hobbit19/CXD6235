...
class PatchMerge(CheckFile,dotProjectDb,checkPatchInfor):
	...
	print 'patchFilename',patchFilename
        ...
        if bugInfo['patchID'] == 'JRD':
            ...
            mailBody.append('<p align=\'Left\'>MTK6580M Platform Pixi4-4 defect %s(<a href="%s">%s </a>)</p>' %(bugInfo['bugID'], bugInfo['bugLink'], bugInfo['bugLink']))
        if bugInfo['patchID'] == 'ALPS':
            mailBody.append('<p align=\'Left\'><b>Dear %s,</b></p>' % dearName)
            mailBody.append('<p align=\'Left\'>MTK6580M Platform Pixi4-4 Patch %s<br/></p>' %bugInfo['bugID'])
    ...
    def __compile(self, CodeDir, branchname):		
        ...
        print "building......"
        if False == self.__debug:
            status = os.system('source build/envsetup.sh && lunch full_jhz6580_weg_m-eng && make -j24 2>&1 |tee android.log')
            print "status is %s" % status            
            if os.path.exists(r"/local/mtk_patch_import/%s/out/target/product/jhz6580_weg_m"  % branchname):
                os.chdir(r'/local/mtk_patch_import/%s/out/target/product/jhz6580_weg_m'  % branchname)
