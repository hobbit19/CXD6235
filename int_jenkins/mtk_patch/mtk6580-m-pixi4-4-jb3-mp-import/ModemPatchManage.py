...
class ModemPatchMerge(CheckFile,dotProjectDb,checkPatchInfor):
    def __init__(self,basedir='/local'):
        ...
        wb = xlrd.open_workbook('%s/mtk6580-m-pixi4-4-jb3-mp-import.xls' % getoutput("dirname %s" % (sys.argv[0])))
        ...
        self.__ModemCode = self.__importCode + "/MT6580M_Modem"
    ...
    def repo_code(self, CodeDir, ModemDir, CodeBranchName):
        ...
        if os.path.exists(CodeDir + "/MT6580M_Modem" + "/.git") == False:
            os.system('git clone git@10.92.32.10:/mtk6580/modem.git MT6580M_Modem')
            os.chdir(ModemDir)
	...
	for branch in self.__DriveOnlyBranch:
            ...
            DriveModemCode = DriveOnlyCode + "/MT6580M_Modem"
    ...
    def __takePatch(self,codedir,patchNo=''):
        ...
        os.system('git rm %s/mtk_rel/JHZ6580_WE_M_GPRS/DEFAULT/tst/database/BPLGUInfoCustomApp*' %codedir)
        os.system('git rm %s/mtk_rel/JHZ6580_WE_M_GPRS/DEFAULT/tst/database/_BPLGUInfoCustomApp*' %codedir)
        os.system('git rm %s/mtk_rel/JHZ6580_WE_M_HSPA/DEFAULT/tst/database/BPLGUInfoCustomApp*' %codedir)
        os.system('git rm %s/mtk_rel/JHZ6580_WE_M_HSPA/DEFAULT/tst/database/_BPLGUInfoCustomApp*' %codedir)
    ...
    def __compile(self, CodeDir):		
        ...
        print "building......"
        if False == self.__debug:
            buildStatus = getstatusoutput("./make.sh \"JHZ6580_WE_M_HSPA.mak\" new")
    ...
    def __copyModemImage(self, CodeDir):
        mdImg = 'mdImg'
        OUT_PATH1 = 'MT6580M_Modem/build/JHZ6580_WE_M_HSPA/DEFAULT/bin'
        OUT_PATH2 = 'MT6580M_Modem/build/JHZ6580_WE_M_HSPA/DEFAULT/tst/database'
        OUT_PATH3 = 'MT6580M_Modem/mtk_rel/JHZ6580_WE_M_HSPA/DEFAULT/tst/database'
        OUT_PATH4 = mdImg
	...
	dearName = sys.argv[4].split('@')[0]
        ...
        for item in range(len(patchList)):
            itemInfo = patchList[item].split('^^^^^^')    			
            gitNamefilder = 'modem'  		
            gitLink = 'http://10.92.32.10/sdd1/gitweb-mtk6580/?p=modem.git;a=commit;h=%s' % itemInfo[1]
		...
		if True == self.getFileLibNvram(self.__ModemCode,projectname):
            ...
            a = u"\\\\10.92.32.12\\RDhzKM\\SWD-Share\\INT\\MTKPatch\\mtk6580m-pixi4-4"
            mailBody.append('<p align=\'Left\'><b>You can also find this patch (P%s) in:<font color="#FF0000">%s</font></b></p>' % (patchNo, a))

            mailBody.append('<p align=\'Left\'><b>After you complete to merge the MTK6580 Pixi4-4 patch P%s,</b></p>' % patchNo)
