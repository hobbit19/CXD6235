...
class PatchMerge(CheckFile,dotProjectDb,checkPatchInfor):
    ...
    def RepoCode(self, CodeDir, BranchName):
        ...
        if os.path.exists(CodeDir + "/.repo") == False:  
            repoinit = pexpect.spawn('%s init -u git@10.92.32.10:sdd3/manifest -m %s.xml' % (self.repopath, BranchName))
	...
	print 'patchFilename',patchFilename
        ...
        if bugInfo['patchID'] == 'JRD':
            ...
            mailBody.append('<p align=\'Left\'>MTK6737 Platform Buzz6t-4G defect %s(<a href="%s">%s </a>)</p>' %(bugInfo['bugID'], bugInfo['bugLink'], bugInfo['bugLink']))

        if bugInfo['patchID'] == 'ALPS':
            mailBody.append('<p align=\'Left\'><b>Dear %s,</b></p>' % dearName)
            mailBody.append('<p align=\'Left\'>MTK6737 Platform Buzz6t-4G Patch %s<br/></p>' %bugInfo['bugID'])
    ...
    def getEmailContent(self,mailBody,patchNo,bugInfo,patchList):
        ...
        for item in range(len(patchList)):
            ...
            if gitName == "kernel-3.18":
                gitName = 'kernel'	
            gitLink = 'http://10.92.32.10/sdd3/gitweb-mtk6737m/?p=%s.git;a=commit;h=%s' % (gitName, itemInfo[1])
    ...
    def __compile(self, CodeDir, branchname):		
        ...
        print "building......"
        if False == self.__debug:
            status = os.system('source build/envsetup.sh && lunch full_jhz6737m_35_b_m0-eng && make -j24 2>&1 |tee android.log')
            print "status is %s" % status            
            if os.path.exists(r"/local/mtk_patch_import/%s/out/target/product/jhz6737m_35_b_m0"  % branchname):
                os.chdir(r'/local/mtk_patch_import/%s/out/target/product/jhz6737m_35_b_m0'  % branchname)
	...
	os.system('scp -r v%s sl_hz_hran@10.92.32.26:/mfs/teleweb/%s/driveronly/' % (version_num,self.branchDict[eachbranch]))
	print "finish upload"		
