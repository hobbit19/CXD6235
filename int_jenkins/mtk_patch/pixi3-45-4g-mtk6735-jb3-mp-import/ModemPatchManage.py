...
class ModemPatchMerge(CheckFile,dotProjectDb,checkPatchInfor):
    def __init__(self,basedir='/local'):
        ...
        self.platform = 'mtk6735'
        self.proj = 'pixi3-45-4g'
        self.dept = 'alps'
        self.sharefoldname = 'pixi3-45-4g'
        self.marketname = 'pixi3-45-4g'
        self.__project = 'JHZ6735M_65C_L'
	self.modem_mak = 'JHZ6735M_65C_L'
    ...
    def repo_code(self, CodeDir, ModemDir, CodeBranchName):
        ...
        if os.path.exists(CodeDir + "/modem" + "/.git") == False:
            os.system('git clone git@10.92.32.10:%s/%s/modem.git modem'%(self.dept,self.platform))
            os.chdir(ModemDir)
	...
	dearName = sys.argv[4].split('@')[0]
        ...
        for item in range(len(patchList)):
            itemInfo = patchList[item].split('^^^^^^')    			
            gitNamefilder = 'modem'  		
            gitLink = 'http://10.92.32.10/%s/gitweb-%s/?p=modem.git;a=commit;h=%s' % self.dept,itemInfo[1]
