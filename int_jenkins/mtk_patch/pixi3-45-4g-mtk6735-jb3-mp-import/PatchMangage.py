...
class PatchMerge(CheckFile,dotProjectDb,checkPatchInfor):
    ...
    def RepoCode(self, CodeDir, BranchName):
        ...
        if os.path.exists(CodeDir + "/.repo") == False:  
            repoinit = pexpect.spawn('%s init -u git@10.92.32.10:%s/manifest -m %s.xml' % (self.repopath,self.manifestdir,BranchName))
	...
	print 'patchFilename',patchFilename
        ...
        if bugInfo['patchID'] == 'JRD':
            ...
            mailBody.append('<p align=\'Left\'>%s Platform shine-plus defect %s(<a href="%s">%s </a>)</p>' %(self.platform,bugInfo['bugID'], bugInfo['bugLink'], bugInfo['bugLink']))

        if bugInfo['patchID'] == 'ALPS':
            mailBody.append('<p align=\'Left\'><b>Dear %s,</b></p>' % dearName)
            mailBody.append('<p align=\'Left\'>%s Platform shine-plus Patch %s<br/></p>' %(self.platform,bugInfo['bugID']))
    ...
    def getEmailContent(self,mailBody,patchNo,bugInfo,patchList):
        ...
        mailBody.append('<p align=\'Left\'><b>After you complete to merge the %s %s patch P%s,</b></p>' % (self.platform,self.proj,patchNo))
    ...
    def __compile(self, CodeDir, branchname):		
        ...
        print "building......"
        if False == self.__debug:
            status = os.system('source build/envsetup.sh && lunch full_%s-eng && make -j24 2>&1 |tee android.log' % 'jhz6735m_35u_o_l')
            print "status is %s" % status            
            if os.path.exists(r"/local/mtk_patch_import/%s/out/target/product/%s"  % (branchname,'jhz6735m_65c_o_l')):
                os.chdir(r'/local/mtk_patch_import/%s/out/target/product/%s'  % (branchname,'jhz6735m_65c_o_l'))
