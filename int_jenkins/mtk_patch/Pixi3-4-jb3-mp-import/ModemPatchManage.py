...
class ModemPatchMerge(CheckFile,dotProjectDb,checkPatchInfor):
	...
	dearName = sys.argv[4].split('@')[0]
        ...
        for item in range(len(patchList)):
            itemInfo = patchList[item].split('^^^^^^')    			
            gitNamefilder = 'modem'  		
            gitLink = 'http://10.92.32.10/sdd1/gitweb-sdd1-all/?p=mtk6572/modem.git;a=commit;h=%s' % itemInfo[1]
		...
		if True == self.getFileLibNvram(self.__ModemCode,projectname):
            ...
            mailBody.append('<p align=\'Left\'><b>After you complete to merge the MTK6572 Pixi3-4/45 modem patch P%s,</b></p>' % patchNo)
