...
class ModemPatchMerge(CheckFile,dotProjectDb,checkPatchInfor):
		...
		self.proj = 'pixi4-5-4g-telstra'
		self.dept = 'sdd2'
		self.manifestdir = 'alps'
		self.sharefoldname = 'pixi4-5-4g-telstra'
		self.marketname = 'pixi4-5-4g-telstra'
		self.__project = 'jhz6735_65t_tl_m'
		self.modem_mak = 'JHZ6735_65T_TL_M'
	...
	def repo_code(self, CodeDir, ModemDir, CodeBranchName):
		...
		if os.path.exists(CodeDir + "/modem" + "/.git") == False:
		    os.system('git clone git@10.92.32.10:%s/%s/modem.git modem'%(self.manifestdir,self.platform))
		    os.chdir(ModemDir)
