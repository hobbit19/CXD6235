...
class PatchMerge(CheckFile,dotProjectDb,checkPatchInfor):
    ...
    def RepoCode(self, CodeDir, BranchName):
        ...
        if os.path.exists(CodeDir + "/.repo") == False:  
            repoinit = pexpect.spawn('%s init -u git@10.92.32.10:%s/manifest -m %s.xml' % (self.manifest,self.repopath, BranchName))
    ...
    def __compile(self, CodeDir, branchname):		
        ...
        print "building......"
        if False == self.__debug:
            status = os.system('source build/envsetup.sh && lunch full_%s-eng && make -j24 2>&1 |tee android.log' % self.__project)
