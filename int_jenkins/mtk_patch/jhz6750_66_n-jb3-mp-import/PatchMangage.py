...
class PatchMerge(CheckFile,dotProjectDb,checkPatchInfor):
	...
	self.repopath = '/local/tools/repo/repo'
        ...
        #wb = xlrd.open_workbook(self.__baseDir + '/mt6580-mp-import.xls')
        ...
        self.__DriveOnlyCode = "/" + "local" + "/mtk_patch_import" +  "/" #+ self.__DriveOnlyBranchName
    ...
    def MergePatch(self):
        ...
        #while (os.path.exists(self.__baseDir + "/start") == True):
        while (os.path.exists("/" + "local" + "/mtk_patch_import" + "/start") == True):
            ...
            if 'Nofile' == getNextPatchNo:
                ...
                continue
            prevMD5 = getoutput('rm -rf /tmp/tempfile; %s forall -c "git add -A; git status" >> /tmp/tempfile; md5sum /tmp/tempfile' % self.repopath).split()[0]
            lunch = 'full_jhz6750_66_n-eng'
            if sys.argv[5].lower() == "yes" and 'Success' == self.__compile(self.__importCode,self.__branch,lunch):
    ...
    def MergeOnePatch(self,patchtype,patchNo=-1):
        ...
        if (os.path.exists("/" + "local" + "/mtk_patch_import"+"/start") == True):
            ...
            patchFilename = self.__takePatch(self.__importCode, patchNo)
            os.system('bash /local/int_jenkins/mtk_patch/lib/updatePjName.sh add jhz6750_66_n elsa6t /local/mtk_patch_import/mtk6750t-n-v0.1-import/')
            os.system('bash /local/int_jenkins/mtk_patch/lib/updatePjName.sh add jhz6750_66_n elsa6p /local/mtk_patch_import/mtk6750t-n-v0.1-import/')
    ...
    def eachDriveOnlyBranchImport(self,DriveOnlygitDir, DriveOnlyCode, eachbranch, patchNo=-1):
		...
		if int(patchNo) % 10 == 0:
			os.system('%s forall -c git tag -a %s -m "%s";%s forall -c git push jgs %s' % (self.repopath,tagname,tagcomment,self.repopath,tagname))
		if len(self.confict_gits) != 0:
			print "cherry pick failed"
			print "you have to check conflicts in the following gits:"
			print self.confict_gits
			sys.exit(1)			
		else:
                	print 'finish merge....'
	...
	print 'patchFilename',patchFilename
        ...
        if bugInfo['patchID'] == 'JRD':
            ...
            mailBody.append('<p align=\'Left\'>MTK6750T Platform ELSA6P-N defect %s(<a href="%s">%s </a>)</p>' %(bugInfo['bugID'], bugInfo['bugLink'], bugInfo['bugLink']))
        if bugInfo['patchID'] == 'ALPS':
            mailBody.append('<p align=\'Left\'><b>Dear %s,</b></p>' % dearName)
            mailBody.append('<p align=\'Left\'>MTK6750T Platform ELSA6P-N Patch %s<br/></p>' %bugInfo['bugID'])
        ...
        if self.nvram_changed == True:
		self.patch_dir = self.patch_dir.encode('utf-8')
		self.emailToList = sys.argv[4]
		os.system('/local/int_jenkins/mtk_patch/lib/MailToCheckFile.py %s %s "%s" "%s" "%s" "%s"' % (self.SPMEmailList[0], self.emailToList,self.__branch, self.vendor_link, patchNo,self.patch_dir))	
    ...
    def getEmailContent(self,mailBody,patchNo,bugInfo,patchList):
		...
		if gitName == "kernel-3.18":
                	gitName = 'kernel'				
		gitLink = 'http://10.92.32.10/sdd1/gitweb-mtk6750t/?p=%s.git;a=commit;h=%s' % (gitName, itemInfo[1])
		...
		if gitName == "vendor":
			...
			if 'sochanged' == self.getFileLibNvram(gitnameDir,sofilename):
				mailBody.append('<p align=\'Left\'><font color="#FF0000">Note: The file of libnvram.os has been changed.Please check the link for detail</font></p>')
				mailBody.append('<p align=\'Left\'><font color="#FF0000"><b>Please contact System team to check whether to merge this modification to project code</font></b></p>')
				self.nvram_changed = True
				self.vendor_link = gitLink
				#os.system('/local/int_jenkins_clean/int_jenkins/mtk_patch/lib/MailToCheckFile.py' %s %s %s %s % (spmlist, emailToList,importbranch, link,patchNo))	  
	...
	self.patch_dir = a
        ...
        mailBody.append('<p align=\'Left\'><b>After you complete to merge the MTK6750 ELSA6P-N patch P%s,</b></p>' % patchNo)
			...
			for item in range(len(patchList)):
				...
				if gitName == folder:
					...
					if "error:" in cerryPickInfo:
						...
						self.confict_gits.append(folder) if folder not in self.confict_gits else ''
					else:
						print "cherry pick successfully"

    def __compile(self, CodeDir, branchname,lunch):		
        ...
        if False == self.__debug:
            ...
            os.system('python compile_modem.py -p elsa6p -b APAC')
            os.chdir(CodeDir)
            status = os.system('source build/envsetup.sh && lunch %s && make -j24 2>&1 |tee android.log' % lunch)
            print "status is %s" % status
            project = ''
            match = re.findall('_(.*)-',lunch)
            if match:
		  project = match[0]
            if not project :
                  print "lunch project error"
                  sys.exit(1)           
            if os.path.exists(r"/local/mtk_patch_import/%s/out/target/product/%s"  % (branchname,project)):
                os.chdir(r'/local/mtk_patch_import/%s/out/target/product/%s'  % (branchname,project))
    ...
    def justBuild(self):
        ...
        self.__reset(self.__importCode)
        lunch = 'full_jhz6750_66_n-eng'
        if 'Fail' == self.__compile(self.__importCode,self.__branch,lunch):
		print "Build Failed"
		sys.exit(1)
	...
	for eachbranch in self.__DriveOnlyBranchName:
		...
		DriveOnlyCode = self.__DriveOnlyCode + eachbranch
        	self.RepoCode(DriveOnlyCode, eachbranch)
        	self.__reset(DriveOnlyCode)
		lunch = 'full_elsa6p-eng'
        	if 'Fail' == self.__compile(DriveOnlyCode, eachbranch,lunch):
	...
	os.chdir(DriveOnlyCode+'/out/target/product/elsa6p')
	tmpstrs=getoutput('ls | grep -E ".bin|.img|_scatter.txt"').split('\n')
	...
	if tmpstrs:
		for tmpstr in tmpstrs:
			os.system('cp -f %s %s'%(tmpstr,driveonly_release_dir+'/v'+version_num))
	#self.__project = "elsa6p"
	#os.system('/local/int_jenkins/sortresult/projects/%s_CopyImg.sh %s %s %s '%(eachbranch, eachbranch, version_num, self.__project))
	print "begin to upload result to teleweb"
	os.chdir(driveonly_release_dir)
	os.system('scp -r v%s sl_hz_hran@10.92.32.26:/mfs/teleweb/%s/driveonly/' % (version_num,'elsa6p'))
	print "finish upload"
