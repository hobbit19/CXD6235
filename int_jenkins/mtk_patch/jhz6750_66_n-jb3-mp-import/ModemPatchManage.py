...
class ModemPatchMerge(CheckFile,dotProjectDb,checkPatchInfor):
    def __init__(self,basedir='/local'):
        ...
        wb = xlrd.open_workbook('%s/jhz6750_66_n-jb3-mp-import.xls' % getoutput("dirname %s" % (sys.argv[0])))
        #wb = xlrd.open_workbook('/local/mtk-patch/mtk-6580-jb3-mp-import//mt6572-jb3-mp-import.xls')
        ...
        self.__sixpatchFormat = mtkSheet.cell(1, 2).value.strip()
        ...
        self.__DriveOnlyBranch = mtkSheet.cell(1, 10).value.strip().split(',')

        self.__importCode = "/" + "local" + "/mtk_patch_import" + "/" + self.__branch + "_MD"
        self.__ModemCode = self.__importCode + "/modem"
        ...
        self.__DriveOnlyCode = "/" + "local" + "/mtk_patch_import" + "/" #+ self.__DriveOnlyBranch + "_MD"
	self.devCodeBranch = []
    ...
    def repo_code(self, CodeDir, ModemDir, CodeBranchName):
        ...
        if os.path.exists(CodeDir + "/modem" + "/.git") == False:
            os.system('git clone git@10.92.32.10:/mtk6750t/modem.git')
            os.chdir(ModemDir)
    ...
    def MergeModemPatch(self):
        ...
        while (os.path.exists("/" + "local" + "/mtk_patch_import" + "/start") == True):
            ...
            band = ''
            if 'Success' == self.__compile(self.__ModemCode,band):
    ...
    def MergeModemOnePatch(self,patchtype,patchNo=-1):
        ...
        if (os.path.exists("/" + "local" + "/mtk_patch_import" + "/start") == True):
            ...
            patchFilename, sixpatchFilename= self.__takePatch(self.__ModemCode, patchNo)
            if 'Nofile' == patchFilename:
                print "No p%s patch. Please copy the correct patch to TODO directory" % patchNo
                sys.exit(1)
            if 'Nosixfile' == patchFilename:
                print "No six p%s patch. Please copy the correct patch to TODO directory" % patchNo
                sys.exit(1)
            os.chdir(self.__ModemCode)
            prevMD5 = getoutput('rm -rf /tmp/tempfile; git add .; git status >> /tmp/tempfile; md5sum /tmp/tempfile').split()[0]
            band = ''
    ...
    def __takePatchForDriveOnly(self, patchNo=''):
        if '' == patchNo:
            ...
            sixpatchFilename = getoutput("ls %s | grep %s | grep P%s\).tar.gz" % (self.__ongoingPatch, self.__sixpatchFormat, patchNo))
        if 0 == len(patchFilename):
            print "Nofile"
        if 0 == len(sixpatchFilename):
            print "Nosixfile"
        return patchFilename,sixpatchFilename
	...
	for branch in self.__DriveOnlyBranch:
            ...
            DriveModemCode = DriveOnlyCode + "/modem"
	...
	if (os.path.exists("/" + "local" + "/mtk_patch_import" + "/start") == True):
            ...
            print "patchNo %s" % patchNo
            patchFilename,sixpatchFilename = self.__takePatchForDriveOnly(patchNo)
            ...
            if True == self.__pushCodeToGit:
                ...
                nvramfile_diff = getoutput("git diff mcu/pcore/custom/band/LATAM2/el1_rf/MT6750_LTE_MT6176_CUSTOM/nvram_editor_data_item.h")
                if nvramfile_diff:
			os.system('git add mcu\/pcore\/custom/band\/LATAM2\/el1_rf\/MT6750_LTE_MT6176_CUSTOM/nvram_editor_data_item.h; git commit -m "change nvram_editor_data_item.h"')
            ...
            #band = ''
            #if 'Success' == self.__compile(DriveModemCode,band):
	...
	os.chdir(codedir)
	os.system('cp mcu/common/interface/service/nvram/nvram_editor_data_item.h mcu/pcore/custom/band/LATAM2/el1_rf/MT6750_LTE_MT6176_CUSTOM/')
	patchformat_for_log = self.__patchFormat.replace('(', '_').replace(')', '_')
	patchList = getoutput('git log origin/%s --format=%s | grep "%s" | grep "porting P%s_" | sort' %(branch, '%s^^^^^^%H^^^^^^$PWD', patchformat_for_log, patchNo)).split('\n')		
	if len(patchList) != 0:
    ...
    def __takePatch(self,codedir,patchNo=''):
        ...
        self.sixuntardir = "%s/MD-P%s/C2K_SIXTH_CBP" % (self.__ongoingPatch,patchNo)
        os.system('mkdir -p %s' % self.untardir)
        os.system('mkdir -p %s' % self.sixuntardir)
        os.system('tar zxvf "%s/%s" -C %s' % (self.__ongoingPatch,patchFilename,self.untardir))
        os.system('tar zxvf "%s/%s" -C %s' % (self.__ongoingPatch,sixpatchFilename,self.sixuntardir))
        if not os.path.isdir(codedir):
            os.system('mkdir -p %s' % codedir)
        
        #os.system('git rm %s/mcu/mtk_rel/JHZ6750_66_N/LWCTG_MP3/dhl/database/pcore_db/MDDB*' %codedir)
        #os.system('git rm %s/mcu/mtk_rel/JHZ6750_66_N/LWCTG_MP3/dhl/database/pcore_db/_MDDB*' %codedir)
        #os.system('git rm %s/mcu/mtk_rel/JHZ6750_66_N/LWCTG_MP3/l1core/dhl/database/MDDB*' %codedir)
        #os.system('git rm %s/mcu/mtk_rel/JHZ6750_66_N/LWCTG_MP3/l1core/dhl/database/_MDDB*' %codedir)
        #os.system('git rm %s/mcu/mtk_rel/JHZ6750_66_N/LWCTG_MP3_50/dhl/database/pcore_db/MDDB*' %codedir)
        #os.system('git rm %s/mcu/mtk_rel/JHZ6750_66_N/LWCTG_MP3_50/dhl/database/pcore_db/_MDDB*' %codedir)
        #os.system('git rm %s/mcu/mtk_rel/JHZ6750_66_N/LWCTG_MP3_50/l1core/dhl/database/MDDB*' %codedir)
        #os.system('git rm %s/mcu/mtk_rel/JHZ6750_66_N/LWCTG_MP3_50/l1core/dhl/database/_MDDB*' %codedir)
        #os.system('git rm %s/mcu/mtk_rel/JHZ6750_66_N/LWCTG_MP3/dsp_rel/MT6750/DEFAULT/final_bin/DbgInfo_DSP_MT6750*' %codedir)
        #os.system('git rm %s/mcu/mtk_rel/JHZ6750_66_N/LWCTG_MP3_50/dsp_rel/MT6750/DEFAULT/final_bin/DbgInfo_DSP_MT6750*' %codedir)
	self.__rmdatabase(codedir,patchNo)
        os.system('cp -dpRv %s/* %s' %(self.untardir,codedir))
        #os.system('rm -rf %s' % untardir)		
        return patchFilename,sixpatchFilename
    def __rmdatabase(self,codedir,patchNo):
		print "========bigin of rm data base========\n"
		print "change dir=======>%s\n" % codedir
		os.chdir(codedir)
		tmp = ''
		tmp = getoutput("find -name '_MDDB*' | grep -v 'P%s'" % patchNo)
		if tmp:
			tmp = tmp.replace('\n',' ')
			print "rm all this files:\n"
			print tmp
			print '\n'
			os.system('git rm %s' % tmp)
		tmp = getoutput("find -name 'MDDB*' | grep -v 'P%s'" % patchNo)
		if tmp:
			tmp = tmp.replace('\n',' ')
			print "rm all this files:\n"
			print tmp
			print '\n'
			os.system('git rm %s' % tmp)
		tmp = getoutput("find -name 'DbgInfo_DSP_MT6750*' | grep -v 'P%s'" % patchNo)
		if tmp:
			tmp = tmp.replace('\n',' ')
			print "rm all this files:\n"
			print tmp
			print '\n'
			os.system('git rm %s' % tmp)
		tmp = getoutput("find -name '_DbgInfo_DSP_MT6750*' | grep -v 'P%s'" % patchNo)
		if tmp:
			tmp = tmp.replace('\n',' ')
			print "rm all this files:\n"
			print tmp
    ...
    def __compile(self, CodeDir, band):		
        ...
        if False == self.__debug:
            if band:
            	buildStatus = getstatusoutput("./compile_modem.py %s" % band)
            else:
            	buildStatus = getstatusoutput("./compile_modem.py")
    ...
    def __copyModemImage(self, CodeDir):
        mdImg = 'mdImg'
        OUT_PATH1 = 'modem/build/JHZ6580_WE_M_HSPA/DEFAULT/bin'
        OUT_PATH2 = 'modem/build/JHZ6580_WE_M_HSPA/DEFAULT/tst/database'
        OUT_PATH3 = 'modem/mtk_rel/JHZ6580_WE_M_HSPA/DEFAULT/tst/database'
        OUT_PATH4 = mdImg
        PATH1 = "JHZ6580_WE_M_HSPA"
        os.chdir(CodeDir)
        os.system('rm -rfv %s'%mdImg)
        os.system('mkdir %s' %mdImg)
        
        os.system('cp %s/JHZ6580_WE_M_HSPA_PCB01_MT6580_S00.elf %s' %(OUT_PATH1,OUT_PATH4))
        os.system('cp %s/JHZ6580_WE_M_HSPA_PCB01_MT6580_S00.MOLY_WR8_W1449_*.bin %s/modem_1_wg_n.img'%(OUT_PATH1,OUT_PATH4))
        os.system('cp %s/JHZ6580_WE_M_HSPA.mak %s/modem_1_wg_n.mak'%(OUT_PATH1,OUT_PATH4))         
        ...
        os.chdir(CodeDir + "/%s" %OUT_PATH4)
        name1 = getoutput('ls -a DbgInfo*')
        os.system('mv %s %s_1_wg_n'%(name1,name1))
        name2 = getoutput('ls -a BPLGUInfoCustomAppSrcP_MT6580_S00_MOLY_WR8_*')
        os.system('mv %s %s_1_wg_n'%(name2,name2))
        os.system('mv %s_PCB01_MT6580_S00.elf %s_PCB01_MT6580_S00_1_wg_n.elf'%(PATH1,PATH1))
        os.system('zip ELF.zip %s_PCB01_MT6580_S00_1_wg_n.elf'%PATH1)
	...
	patchformat_for_log = self.__patchFormat.replace('(', '_').replace(')', '_')
        patchList = getoutput('git log origin/%s --format=%s\ | grep "%s" | grep "porting P%s_" | sort' %(self.__branch, '%s^^^^^^%H^^^^^^$PWD', patchformat_for_log, patchNo)).split('\n')
	...
	dearName = sys.argv[4].split('@')[0]
        ...
        for item in range(len(patchList)):
            itemInfo = patchList[item].split('^^^^^^')    			
            gitNamefilder = 'modem'  		
            gitLink = 'http://10.92.32.10/sdd1/gitweb-mtk6750t/?p=modem.git;a=commit;h=%s' % itemInfo[1]
		...
		if self.checkFileDict[keyname]['gitname'] == 'modem':
			if True == self.getFileLibNvram(self.__ModemCode,filename):
				mailBody.append('<p align=\'Left\'><font color="blue"><b>Note: The file of %s has been changed in patch P%s.</font></b></p>' % (filename,patchNo))
				mailBody.append('<p align=\'Left\'><font color="blue"><b>Please contact Drive team to check whether to merge this modification to project code</font></b></p>')
				mailBody.append('<p align=\'Left\'><font color="blue"><b>Or to make sure which file should be merged in fact.</font></b></p>')
				mailBody.append('<br/>')		
            ...
            a = u"\\\\10.92.32.12\\RDhzKM\\SWD-Share\\INT\\MTKPatch\\elsa6p"
            mailBody.append('<p align=\'Left\'><b>You can also find these patches (P%s) in:<font color="#FF0000">%s</font></b></p>' % (patchNo, a))
            mailBody.append('<p align=\'Left\'><b>Include:</b></p>')
            mailBody.append('<p align=\'Left\'><b>Moly patch:<font color="#FF0000">%s</font></b></p>' % patchFilename)
            mailBody.append('<p align=\'Left\'><b>Sixth patch:<font color="#FF0000">%s</font></b></p>' % sixpatchFilename)
            mailBody.append('<p align=\'Left\'><b>After you complete to merge the MTK6750T ELSA6T patch P%s,</b></p>' % patchNo)
