...
class ModemPatchMerge(CheckFile,dotProjectDb,checkPatchInfor):
    def __init__(self,basedir='/local'):
        ...
        self.platform = 'mtk6737m'
        self.proj = 'shine-lite-cn'
        self.dept = 'sdd2'
        self.sharefoldname = 'shine-lite-cn'
        self.marketname = 'shine-lite-cn'
        self.__project = 'jhz6737_65_cn'
	self.modem_mak = 'JHZ6737_65_CN_M0'
        wb = xlrd.open_workbook('%s/%s-%s-jb3-mp-import.xls' % (getoutput("dirname %s" % (sys.argv[0])),self.proj,self.platform))
        mtkSheet = wb.sheet_by_name(u'ModemInfo')
        self.__branch = mtkSheet.cell(1, 0).value.strip()
        self.__patchFormat = (mtkSheet.cell(1, 1).value.strip(),mtkSheet.cell(1, 2).value.strip())
        ...
        self.__ModemCode = (self.__importCode + "/modem/LWTG_MOLY_LR9",self.__importCode + "/modem/C2K_SIXTH_CBP")
	...
	self.projectList = self.checkProjectName()
        ...
        for patchFormat in self.__patchFormat:
           print "baseDir = %s\nbranch = %s\npatchFormat = %s\nmailServer = %s\nmailTitle = %s\nmailAccount = %s\nmailSender = %s\nongoingPatch = %s\nDriveOnlyBranchName = %s" % (self.__baseDir,self.__branch, patchFormat, self.__mailServer, self.__mailTitle, self.__mailAccount,self.__mailSender, self.__ongoingPatch,self.__DriveOnlyBranch)
    ...
    def repo_code(self, CodeDir, ModemDir, CodeBranchName):
        ...
        os.chdir(CodeDir)
        print ModemDir
        if os.path.exists(ModemDir + "/.git") == False:
            os.system('git clone git@10.92.32.10:%s/%s.git %s'%(self.platform,ModemDir,ModemDir))
            os.chdir(ModemDir)
    ...
    def MergeModemPatch(self):
        ...
        for ModemCode in self.__ModemCode:
          self.repo_code(self.__importCode, ModemCode, self.__branch)
          while (os.path.exists("/" + "local" + "/mtk_patch_import" + "/start") == True):
            self.__reset(ModemCode)
            patchNo = self.__getNextPatchNo(ModemCode)
            print "getNextPatchNo %s" % patchNo
            patchFilename = self.__takePatch(ModemCode, patchNo)
            ...
            if 'Success' == self.__compile(ModemCode):
                ...
                if True == self.__pushCodeToGit:
                    ...
                    if prevMD5 == nextMD5:
                        ...
                        self.__movePatchToMergeDone(patchNo,ModemCode)
                    print 'finish merge....'
    ...
    def __takePatchForDriveOnly(self, patchNo='',patchFormat=''):
        if '' == patchNo:
            print "patchNo is none"
            sys.exit(1)
        elif '0' == patchNo:            
            match_patch = re.match(r'For_(.*)', patchFormat) 
            if match_patch:
                patch_format = match_patch.group(1)
                patchFilename = getoutput("ls %s | grep %s.tar.gz" % (self.__ongoingPatch, patch_format))
        else:
            patchFilename = getoutput("ls %s | grep %s | grep P%s\).tar.gz" % (self.__ongoingPatch, patchFormat, patchNo))
	...
	for branch in self.__DriveOnlyBranch:
          for ModemCode in self.__ModemCode:
            ...
            DriveModemCode = DriveOnlyCode + ModemCode
	...
	print "Please create file(start) on /local/mtk_patch_import directory. If not exist, the script will stop and exit."  
	  ...
	  if (os.path.exists("/" + "local" + "/mtk_patch_import" + "/start") == True):
            ...
            print "patchNo %s" % patchNo
            patchFilename = self.__takePatchForDriveOnly(patchNo,patchFormat)
            ...
            if 'Success' == self.__compile(DriveModemCode):
	...
	os.chdir(codedir)
        print codedir
        if "C2K_SIXTH_CBP" in codedir:
           patchFormat = self.__patchFormat[1]
           print patchFormat
	patchList = getoutput('git log origin/%s --format=%s | grep "%s" | grep "porting P%s_" | sort' %(branch, '%s^^^^^^%H^^^^^^$PWD', patchFormat, patchNo)).split('\n')		
	if len(patchList) != 0:
    ...
    def __getNextPatchNo(self,modeCode):
        PatchNo = 0
        os.chdir(modeCode)
        if "C2K_SIXTH_CBP" in modeCode:
           patchFormat = self.__patchFormat[1]
           print patchFormat
        patchList = getoutput('git log origin/%s --format=%s | grep %s' %(self.__branch, '%s', patchFormat)).split('\n')
    ...
    def __takePatch(self,codedir,patchNo=''):
        if "C2K_SIXTH_CBP" in codedir:
            ...
            patchFormat = self.__patchFormat[0]
        ...
        os.system('git rm %s/mtk_rel/%s/LTG_DSDS/dhl/database/BPLGUInfoCustomApp*' %(codedir,self.modem_mak))
        os.system('git rm %s/mtk_rel/%s/LTG_DSDS/dhl/database/_BPLGUInfoCustomApp*' %(codedir,self.modem_mak))
        os.system('git rm %s/mtk_rel/%s/LTG_DSDS_COTSX/dhl/database/BPLGUInfoCustomApp*' %(codedir,self.modem_mak))
        os.system('git rm %s/mtk_rel/%s/LTG_DSDS_COTSX/dhl/database/_BPLGUInfoCustomApp*' %(codedir,self.modem_mak))
        os.system('git rm %s/mtk_rel/%s/LWG_DSDS/dhl/database/BPLGUInfoCustomApp*' %(codedir,self.modem_mak))
        os.system('git rm %s/mtk_rel/%s/LWG_DSDS/dhl/database/_BPLGUInfoCustomApp*' %(codedir,self.modem_mak))
        os.system('git rm %s/mtk_rel/%s/LWG_DSDS_COTSX/dhl/database/BPLGUInfoCustomApp*' %(codedir,self.modem_mak))
        os.system('git rm %s/mtk_rel/%s/LWG_DSDS_COTSX/dhl/database/_BPLGUInfoCustomApp*' %(codedir,self.modem_mak))
        os.system('git rm %s/mtk_rel/%s/LWG_TSTS/dhl/database/BPLGUInfoCustomApp*' %(codedir,self.modem_mak))
        os.system('git rm %s/mtk_rel/%s/LWG_TSTS/dhl/database/_BPLGUInfoCustomApp*' %(codedir,self.modem_mak))

        os.system('cp -dpRv %s/* %s' %(self.untardir,codedir))
        #os.system('rm -rf %s' % untardir)		
        return patchFilename
    
    def __movePatchToMergeDone(self, patchNo,ModemCode):
        os.system('mkdir -p %s' % self.__mergeDone)
        if "C2K_SIXTH_CBP" in ModemCode:
            patchFormat = self.__patchFormat[1]
            print patchFormat 
        if '0' == patchNo:
            match_patch = re.match(r'For_(.*)', patchFormat) 
            ...
            os.system('mv %s/*%s*.P%s\).tar.gz %s' % (self.__ongoingPatch,patchFormat,patchNo,self.__mergeDone))


    def __compile(self, CodeDir):
        if "C2K_SIXTH_CBP" in CodeDir:
            print "can not build C2K modem"
            return 'Success' 
        ...
        print "building......"
        if False == self.__debug:
            buildStatus = getstatusoutput("./make.sh \"%s(LWG_DSDS).mak\" new"%self.modem_mak)
	...
	dearName = sys.argv[4].split('@')[0]
        ...
        for patchFormat in self.__patchFormat:
	  print self.__ongoingPatch, patchFormat, patchNo
          patchFilename = getoutput("ls %s | grep %s | grep P%s\).tar.gz" % (self.__ongoingPatch, patchFormat, patchNo))
	  print 'patchFilename',patchFilename
          self.patch_type,self.vnum,self.pnum,self.eservice_ID = self.getMtkPatchInfor(patchFilename) 
          if not os.path.isdir(self.__mailDir):
            os.system('mkdir -p %s' % self.__mailDir)
          if "C2K" in patchFormat:
            ModemCode = self.__ModemCode[1]           
          else:
            ModemCode=self.__ModemCode[0]  
          print ModemCode    
          os.chdir(ModemCode)
          #Please don't add 'repo sync' action here, it will affect merge work.
          patchList = getoutput('git log origin/%s --format=%s\ | grep "%s" | grep "porting P%s_" | sort' %(self.__branch, '%s^^^^^^%H^^^^^^$PWD', patchFormat, patchNo)).split('\n')
		...
		print "The pacth has no modification"
          ...
          for item in range(len(patchList)):
            itemInfo = patchList[item].split('^^^^^^')    			
            gitNamefilder = 'modem'  		
            gitLink = 'http://10.92.32.10/%s/gitweb-%s/?p=modem.git;a=commit;h=%s' % (self.dept,self.platform,itemInfo[1])
        ...
        mailBody.append('<p align=\'Left\'><b>After you complete to merge the %s %s patch P%s,</b></p>' % (self.platform,self.marketname,patchNo))
	...
	for branch in self.__DriveOnlyBranch:
            ...
            os.chdir(DriveModemCode)
            patchList = getoutput('git log origin/%s --format=%s\ | grep "%s" | grep "porting P%s_" | sort' %(branch, '%s^^^^^^%H^^^^^^$PWD', self.__patchFormat, patchNo)).split('\n')
            for item in range(len(patchList)):
            	itemInfo = patchList[item].split('^^^^^^')
            	if 3 != len(itemInfo):
                	print "patchList have problems. patchList = %s" % patchList
