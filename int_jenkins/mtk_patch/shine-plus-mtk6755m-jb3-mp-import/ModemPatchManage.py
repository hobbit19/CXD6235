...
class ModemPatchMerge(CheckFile,dotProjectDb,checkPatchInfor):
    def __init__(self,basedir='/local'):
        ...
        self.platform = 'mtk6755m'
        self.proj = 'shine-plus'
        self.dept = 'sdd2'
        self.marketname = 'shine-plus'
        self.__project = 'JHZ6755_66_M'
    ...
    def __takePatch(self,codedir,patchNo=''):
        ...
        if not os.path.isdir(codedir):
            os.system('mkdir -p %s' % codedir)
	tmpdir = os.getcwd()
        tmp = getoutput("ls %s/mcu/mtk_rel/%s/LWCTG/dhl/database/pcore_db | grep -v 'P%s'"%(codedir,self.__project,patchNo))
	tmp = tmp.replace('\n',' ')
	print 'tmp===%s\n'%tmp
	print 'change dir ======>%s/mcu/mtk_rel/%s/LWCTG/dhl/database/pcore_db\n'%(codedir,self.__project)
	os.chdir('%s/mcu/mtk_rel/%s/LWCTG/dhl/database/pcore_db'%(codedir,self.__project))
        os.system('git rm %s' %tmp)
	tmp = getoutput("ls %s/mcu/mtk_rel/%s/LWCTG/dsp_rel/MT6755/DEFAULT/final_bin | grep -v 'P%s' | grep 'DbgInfo_DSP_MT6755_MOLY_LR11_W1539_MD_MP_V9'"%(codedir,self.__project,patchNo))
	tmp = tmp.replace('\n',' ')
	print 'tmp===%s\n'%tmp
	print 'change dir ======>%s/mcu/mtk_rel/%s/LWCTG/dsp_rel/MT6755/DEFAULT/final_bin\n'%(codedir,self.__project)
	os.chdir('%s/mcu/mtk_rel/%s/LWCTG/dsp_rel/MT6755/DEFAULT/final_bin'%(codedir,self.__project))
        os.system('git rm %s' %tmp)
	tmp = getoutput("ls %s/mcu/mtk_rel/%s/LWCTG/l1core/dhl/database | grep -v 'P%s'"%(codedir,self.__project,patchNo))
	tmp = tmp.replace('\n',' ')
	print 'tmp===%s\n'%tmp
	print 'change dir ======>%s/mcu/mtk_rel/%s/LWCTG/l1core/dhl/database\n'%(codedir,self.__project)
	os.chdir('%s/mcu/mtk_rel/%s/LWCTG/l1core/dhl/database'%(codedir,self.__project))
        os.system('git rm %s' %tmp)
        os.chdir(tmpdir)
    ...
    def __compile(self, CodeDir):		
        ...
        if (space < 5):
            ...
            return "Fail"
	print "change dir =======>%s\n"%CodeDir
        os.chdir(CodeDir+'/mcu')
        os.system('chmod 777 ./tools/init/strcmpex_linux.exe')
        print "building......"
        if False == self.__debug:
            buildStatus = getstatusoutput("./m \"%s(LWCTG).mak\" new"%self.__project)
	...
	dearName = sys.argv[4].split('@')[0]
		...
		if True == self.getFileLibNvram(self.__ModemCode,projectname):
            ...
            a = u"\\\\10.92.32.12\\RDhzKM\\SWD-Share\\INT\\MTKPatch\\%s" % self.proj
