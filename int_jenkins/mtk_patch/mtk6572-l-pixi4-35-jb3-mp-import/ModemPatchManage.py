...
class ModemPatchMerge(CheckFile,dotProjectDb,checkPatchInfor):
	...
	self.sharefoldname = 'pixi4-35'
        wb = xlrd.open_workbook('%s/mtk6572-l-pixi4-35-jb3-mp-import.xls' % getoutput("dirname %s" % (sys.argv[0])))
        ...
        self.__mergedBranchList = mtkSheet.cell(1, 11).value.strip().split(',')
        self.__platform = mtkSheet.cell(1, 12).value.strip()
        self.__projectname = mtkSheet.cell(1, 13).value.strip()
        self.__makefile = mtkSheet.cell(1, 14).value.strip()



        self.__importCode = "/" + "local" + "/mtk_patch_import" + "/" + self.__branch + "_MD"
        self.__ModemCode = self.__importCode + "/MT6572L_Modem"
    ...
    def repo_code(self, CodeDir, ModemDir, CodeBranchName, CloneDir):
        print "Now checking the modem code if exist,"
        if not os.path.isdir(CodeDir):
            os.system('mkdir -p %s' % CodeDir)
        os.chdir(CodeDir)

        if os.path.exists(CodeDir + "/MT6572L_Modem" + "/.git") == False:
            os.system('git clone git@10.92.32.10:%s MT6572L_Modem' % CloneDir)
            os.chdir(ModemDir)
    ...
    def MergeModemOnePatch(self,patchtype,patchNo=-1):
        print "Please create file(start) on /local/mtk_patch_import directory. If not exist, the script will stop and exit."
	CloneDir = "sdd1/odm/modem"
        self.repo_code(self.__importCode, self.__ModemCode, self.__branch, CloneDir)
	...
	for branch in self.__DriveOnlyBranch:
            ...
            DriveModemCode = DriveOnlyCode + "/MT6572L_Modem"
            self.repo_code(DriveOnlyCode,DriveModemCode,branch,CloneDir) 
    ...
    def __takePatch(self,codedir,patchNo=''):
        ...
        os.system('git rm %s/mtk_rel/JHZ6572_WET_L_GPRS/DEFAULT/tst/database/BPLGUInfoCustomApp*' %codedir)
        os.system('git rm %s/mtk_rel/JHZ6572_WET_L_GPRS/DEFAULT/tst/database/_BPLGUInfoCustomApp*' %codedir)
        os.system('git rm %s/mtk_rel/JHZ6572_WET_L_HSPA/DEFAULT/tst/database/BPLGUInfoCustomApp*' %codedir)
        os.system('git rm %s/mtk_rel/JHZ6572_WET_L_HSPA/DEFAULT/tst/database/_BPLGUInfoCustomApp*' %codedir)
    ...
    def __compile(self, CodeDir):		
        ...
        print "building......"
        if False == self.__debug:
            buildStatus = getstatusoutput("./make.sh \"JHZ6572_WET_L_HSPA.mak\" new")
    ...
    def __copyModemImage(self):
        mdImg = 'mdImg'
        OUT_PATH1 = 'MT6572L_Modem/build/JHZ6580_WE_L_HSPA/DEFAULT/bin'
        OUT_PATH2 = 'MT6572L_Modem/build/JHZ6580_WE_L_HSPA/DEFAULT/tst/database'
        OUT_PATH3 = 'MT6572L_Modem/mtk_rel/JHZ6580_WE_L_HSPA/DEFAULT/tst/database'
        OUT_PATH4 = mdImg
        PATH1 = "JHZ6580_WE_L_HSPA"
        os.chdir(self.__importCode)
        os.system('rm -rfv %s'%mdImg)
        os.system('mkdir %s' %mdImg)
        
        os.system('cp %s/JHZ6580_WE_L_HSPA_PCB01_MT6580_S00.elf %s' %(OUT_PATH1,OUT_PATH4))
        os.system('cp %s/JHZ6580_WE_L_HSPA_PCB01_MT6580_S00.MOLY_WR8_W1449_*.bin %s/modem_1_wg_n.img'%(OUT_PATH1,OUT_PATH4))
        os.system('cp %s/JHZ6580_WE_L_HSPA.mak %s/modem_1_wg_n.mak'%(OUT_PATH1,OUT_PATH4))         
	...
	dearName = sys.argv[4].split('@')[0]
		...
		if True == self.getFileLibNvram(self.__ModemCode,projectname):
            ...
            mailBody.append('<p align=\'Left\'><b>After you complete to merge the MTK6572 Pixi4-35 patch P%s,</b></p>' % patchNo)
