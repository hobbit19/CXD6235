...
class PatchMerge(CheckFile,dotProjectDb,checkPatchInfor):
	...
	self.sharefoldname = 'pixi4-35'
        ...
        self.__mergedBranchList = mtkSheet.cell(1, 12).value.strip().split(',')
        self.__platform = mtkSheet.cell(1, 13).value.strip()
        self.__projectname = mtkSheet.cell(1, 14).value.strip()
        self.__lunch = mtkSheet.cell(1, 15).value.strip()
    ...
    def MergeOnePatch(self,patchtype,patchNo=-1):
        ...
        manifest = "odm_manifest"
        self.RepoCode(self.__importCode, self.__branch, manifest)
        print "Please create file(start) on /local/mtk_patch_import directory. If not exist, the script will stop and exit."
    ...
    def RepoCode(self, CodeDir, BranchName, manifest):
        ...
        if os.path.exists(CodeDir + "/.repo") == False:  
            repoinit = pexpect.spawn('%s init -u git@10.92.32.10:sdd1/%s -m %s.xml' % (self.repopath, manifest, BranchName))
	...
	for eachbranch in self.__DriveOnlyBranchName:
		...
		self.RepoCode(DriveOnlyCode, eachbranch, manifest)
    ...
    def eachDriveOnlyBranchImport(self,DriveOnlygitDir, DriveOnlyCode, eachbranch, patchNo=-1):
        ...
        if (os.path.exists("/" + "local" + "/mtk_patch_import"+"/start") == True):
            ...
            tagname = 'porting_mtk_patch_%s_for_DOnly' % patchFilename.replace('(', '_').replace(')', '_')
	...
	print 'patchFilename',patchFilename
        ...
        if bugInfo['patchID'] == 'JRD':
            ...
            mailBody.append('<p align=\'Left\'>%s Platform %s defect %s(<a href="%s">%s </a>)</p>' %(self.__platform, self.__projectname, bugInfo['bugID'], bugInfo['bugLink'], bugInfo['bugLink']))
        if bugInfo['patchID'] == 'ALPS':
            mailBody.append('<p align=\'Left\'><b>Dear %s,</b></p>' % dearName)
            mailBody.append('<p align=\'Left\'>%s Platform %s Patch %s<br/></p>' %(self.__platform, self.__projectname,bugInfo['bugID']))
	...
	for item in range(len(patchList)):
            ...
            if gitName == "kernel-3.10":
                gitName = 'kernel'
            gitLink = 'http://10.92.32.10/sdd1/gitweb-odm/?p=%s.git;a=commit;h=%s' % (gitName, itemInfo[1])
	...
	mailBody.append('<p align=\'Left\'><b>After you complete to merge the %s %s patch P%s,</b></p>' % (self.__platform, self.__projectname, patchNo))
    ...
    def __compile(self, CodeDir, branchname):		
        ...
        print "building......"
        if False == self.__debug:
            status = os.system('source build/envsetup.sh && lunch %s && make -j24 2>&1 |tee android.log' % self.__lunch)
