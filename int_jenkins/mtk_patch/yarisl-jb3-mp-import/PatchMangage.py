...
class PatchMerge(CheckFile,dotProjectDb,checkPatchInfor):
	...
	self.repopath = '/local/tools/repo/repo'
        ...
        #self.__importCode = self.__baseDir + "/" + self.__branch
	...
	self.devCodeBranch = self.getDevBranchNameFromIProjectID(self.devCodeProjectIDList,self.projectid_codeBranch_Dict)
        ...
        if os.path.exists(self.__importCode + "/.repo") == False:
            ...
            repoinit = pexpect.spawn('%s init -u git@10.92.32.10:sdd1/manifest -m sync/%s.xml' % (self.repopath, self.__branch))
    ...
    def MergeOnePatch(self,patchNo=-1):
        ...
        if int(patchNo) == -1:
            ...
            return
        #if (os.path.exists(self.__baseDir + "/start") == True):
	...
	mailBody.append('<p align=\'Left\'>MTK Patch <b><font color="#FF0000">P%s</font></b> has been merged to import branch(mt6572-jb2-import)<br/></p>' % patchNo)
	...
	for item in range(len(patchList)):
		itemInfo = patchList[item].split('^^^^^^')
            ...
            mailBody.append('<p align=\'Left\'><b>You can also find this patch (P%s) in:<font color="#FF0000">\\\\rd-filebackup\\RDhzKM\\4-SDD1\\B-\BC\BC\CA\F5\CEĵ\B5\\A-MTKԴ\B4\FA\C2\EB\\6572\\ALPS.KK1.MP6.V1_JRDHZ72_WE_72_S1_KK_INHOUSE</font></b></p>' %patchNo)
...
if __name__ == "__main__":
    ...
    elif len(sys.argv) == 6 and sys.argv[2] == 'PatchMail':
        pm = PatchMerge(sys.argv[1])
