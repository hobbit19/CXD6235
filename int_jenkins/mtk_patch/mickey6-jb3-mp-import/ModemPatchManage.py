...
class ModemPatchMerge(CheckFile,dotProjectDb,checkPatchInfor):
	...
	print "change dir ========>",basedir
        os.chdir(basedir)
        self.__baseDir = os.getcwd()		
        self.__debug = False
        self.__pushCodeToGit = True

        wb = xlrd.open_workbook('%s/mickey6-jb3-mp-import.xls' % getoutput("dirname %s" % (sys.argv[0])))
    ...
    def repo_code(self, CodeDir, ModemDir, CodeBranchName):
        ...
        if os.path.exists(CodeDir + "/modem" + "/.git") == False:
            os.system('git clone git@10.92.32.10:mtk6737/modem.git modem')
            os.chdir(ModemDir)
    ...
    def __takePatch(self,codedir,patchNo=''):
        ...
        #os.system('git rm %s/mtk_rel/JHZ6737T_65_MI_M0/LTG_DSDS/dhl/database/BPLGUInfoCustomApp*' %codedir)
        #os.system('git rm %s/mtk_rel/JHZ6737T_65_MI_M0/LTG_DSDS/dhl/database/_BPLGUInfoCustomApp*' %codedir)
        #os.system('git rm %s/mtk_rel/JHZ6737T_65_MI_M0/LTG_DSDS_COTSX/dhl/database/BPLGUInfoCustomApp*' %codedir)
        #os.system('git rm %s/mtk_rel/JHZ6737T_65_MI_M0/LTG_DSDS_COTSX/dhl/database/_BPLGUInfoCustomApp*' %codedir)
        #os.system('git rm %s/mtk_rel/JHZ6737T_65_MI_M0/LWG_DSDS/dhl/database/BPLGUInfoCustomApp*' %codedir)
        #os.system('git rm %s/mtk_rel/JHZ6737T_65_MI_M0/LWG_DSDS/dhl/database/_BPLGUInfoCustomApp*' %codedir)
        #os.system('git rm %s/mtk_rel/JHZ6737T_65_MI_M0/LWG_DSDS_COTSX/dhl/database/BPLGUInfoCustomApp*' %codedir)
        #os.system('git rm %s/mtk_rel/JHZ6737T_65_MI_M0/LWG_DSDS_COTSX/dhl/database/_BPLGUInfoCustomApp*' %codedir)
        #os.system('git rm %s/mtk_rel/JHZ6737T_65_MI_M0/LWG_TSTS/dhl/database/BPLGUInfoCustomApp*' %codedir)
        #os.system('git rm %s/mtk_rel/JHZ6737T_65_MI_M0/LWG_TSTS/dhl/database/_BPLGUInfoCustomApp*' %codedir)
    ...
    def __compile(self, CodeDir):		
        ...
        print "building......"
        if False == self.__debug:
            buildStatus = getstatusoutput("./make.sh \"JHZ6737T_65_MI_M0(LWG_TSTS).mak\" new")
	...
	dearName = sys.argv[4].split('@')[0]
        ...
        for item in range(len(patchList)):
            itemInfo = patchList[item].split('^^^^^^')    			
            gitNamefilder = 'modem'  		
            gitLink = 'http://10.92.32.10/sdd2/gitweb-mtk6737/?p=modem.git;a=commit;h=%s' % itemInfo[1]
		...
		if True == self.getFileLibNvram(self.__ModemCode,projectname):
            ...
            a = u"\\\\10.92.32.12\\RDhzKM\\SWD-Share\\INT\\MTKPatch\\mickey6"
            mailBody.append('<p align=\'Left\'><b>You can also find this patch (P%s) in:<font color="#FF0000">%s</font></b></p>' % (patchNo, a))

            mailBody.append('<p align=\'Left\'><b>After you complete to merge the MTK6737 Mickey6 patch P%s,</b></p>' % patchNo)
